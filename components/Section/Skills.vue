				...
				<div grid grid-cols-1 md="grid-cols-12" gap-10>
					...
					<div md="col-span-7 col-start-6">
						<div grid grid-cols-2 md="grid-cols-3" lg="grid-cols-4" gap-2>
							<div v-for="(skill, skillIndex) in skills" :key="skill.id" class="group cell" :style="`animation-delay: ${100 * skillIndex}ms`" invisible relative aspect-square h-full w-full flex-center gradient overflow-hidden rounded-xl>
								<NuxtImg class="group-hover:opacity-0" :src="skill.image" :alt="skill.title" size-15 transition-opacity duration-250 />
